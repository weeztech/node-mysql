...
Query.prototype['ResultSetHeaderPacket'] = function (packet) {
    if (packet.fieldCount === null) {
        ...
        var rowsAsArray = this.rowsAsArray,rows = this._resultSet.rows;
        if (rowsAsArray) {
            if (Array.isArray(rowsAsArray)) {
                rowsAsArray = rowsAsArray[this._resultSetIndex]
            }
        }
        rows.rowsAsArray = rowsAsArray;
    }
};
...
Query.prototype['RowDataPacket'] = function (packet, parser, connection) {
    packet.parse(parser, this._resultSet.fieldPackets, this.typeCast, this.nestTables, this._resultSet.rows.rowsAsArray, connection);

    if (this._callback) {
        this._resultSet.rows.push(packet.row || packet);
    } else {
        this.emit('result', packet.row || packet, this._index);
    }
};
