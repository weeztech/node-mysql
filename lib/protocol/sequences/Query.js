...
var RowArrayDataPacket = require('../packets/RowArrayDataPacket');
...
Query.prototype.determinePacket = function(firstByte, parser) {
  ...
  if (firstByte === 255) {
    ...
    return (this._resultSet.eofPackets.length === 0)
        ? Packets.FieldPacket
        : (!this.nestTables && this.rowsAsArray)
        ? RowArrayDataPacket
...
Query.prototype['RowArrayDataPacket'] = function (packet, parser, connection) {
    packet.parse(parser, this._resultSet.fieldPackets, this.typeCast, connection);

    if (this._callback) {
        this._resultSet.rows.push(packet.row);
    } else {
        this.emit('result', packet.row, this._index);
    }
};
